...
COMPANY = 'Warp'
REPO = 'notice-fixture'
PROJECT_TERMS = ['central', 'claim', 'agents', 'registrations', 'resolve', 'notices', 'handle', 'quarterly']
PROJECT_METRICS = ['central_coverage', 'claim_risk', 'agents_precision', 'registrations_latency']
PROJECT_FAILURES = ['central_drift', 'claim_gap', 'agents_misroute', 'registrations_blindspot']
PROJECT_ARCHETYPES = [{'name': 'central evidence replay', 'trigger': 'central signal changes while claim context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'claim boundary probe', 'trigger': 'claim handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'agents regression harness', 'trigger': 'agents behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'registrations operator packet', 'trigger': 'registrations output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = "A per jurisdiction, per form regression harness for tax compliance agents — the release gate that lets Warp's AI agents file faster and prove it."
...
HERO_TITLE = 'Warp Adversarial Replay Board'
